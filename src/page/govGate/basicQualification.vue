<template>
    <div class="wrapper">
        ...
        <!-- <div class="layouts">
            <itemTab
                    :breadcrumb="[{url: `index?uid=${$route.query.uid}`,title:'首页'},{url: `agencyBasic?uid=${$route.query.uid}`,title:'基本信息'},{title:'资质证书'}]"
            ></itemTab>
        </div>

        <divider solid style="margin:0 0 30px 0" /> -->
        <div class="tc mb50">
                <h5 class="mt30">资质证书</h5>
                <p class="t-grey mt10">Qualification certificate</p>
        </div>
        ...
        <div class="layouts">
            <div  v-if="data.length > 0" class="rural-pro-list">
                <Row type="flex" :gutter="20" class="pb10">
                    ...
                    <!-- <div>
                        <h5 class="mt10 mb30">{{item.columns}}</h5>
                    </div> -->
                    </Col>
                </Row>
                <Page class="tc country pt20 pb50" :page-size="page.pageSize" :total="total" @on-change="nextPage"></Page>
            </div>
