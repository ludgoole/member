    ...
    <div class="vui-member-header">
        <Row type="flex" align="middle" class="layouts">
            ...
            <Col span="12">
                    <Row>
                        <Col span="5">
                            <div class="user-name" v-if="displayName !== ''">{{displayName}}</div>
                        </Col>
                    </Row>
                    ...
                    <Row class="mt15">
	                    <Avatar icon="card" size="small" style="background-color: #19be6b" @click.native="handleRestartAuthen"/>
	                    <span class="ns-avas mr10" >{{authen}}</span>
                    	<Avatar icon="social-vimeo" size="small" style="background-color: #ff9900" />
    ...
    created () {
        this.$api.post('/member/login/findCurrentUser', {
            ...
            if (res.data.userType === 0 || res.data.userType === 4) {
                if (res.data.isIdentityVerification !== '0') {
                    this.authen = '已实名'
                } else {
                    this.authen = '未实名'
                }
