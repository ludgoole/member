                                ...
                                <Form ref="corpInfo" :model="corpInfo" label-position="right" :label-width="280" :rules="ruleInline">
                                    <div style="width: 1000px;" v-if="step1">
                                        ...
                                        <Form-item prop="logo_picture_list" class="logo-pic" label="上传企业LOGO：">
                                            ...
                                            <div class="new-upload-list" v-for="(item,index) in corpInfo.logo_picture_list" :key="index">
                                                <template v-if="item.status === 'finished'">
                                                    <img style="width:140px; height: 140px;" :src="'http://'+item.response.data.picName">
                                                    <div class="demo-upload-list-cover">
                                                        <Icon type="ios-trash-outline" size="30" style="margin-top: 55px;" @click.native="handleRemove3(item)"></Icon>
                                                    </div>
                                                </template>
                                                <template v-else>
...
<script>
    ...
    export default {
        ...
        created () {
            ...
            // this.getAlbum('logoPhoto', this.l3)
        ...
        methods: {
            ...
            handleGetPhotoResult3 (result) {
                // 用于限制最大可上传照片的张数
                if (this.corpInfo.logo_picture_list.length + result.length > 1) {
                    this.$Notice.warning({
                        title: '最多只能上传 1 张图片。'
                    })
                } else {
                    result.forEach(element => {
                        this.corpInfo.logo_picture_list.push({
                            response: {
                                data: {
                                    picName: element
                                }
            ...
            handleBeforeUpload3 () {
                const check = this.$refs.upload3.fileList.length < 1 &&
                this.corpInfo.logo_picture_list.length + this.$refs.upload3.fileList.length < 1
                if (!check) {
                    this.$Notice.warning({
                        title: '最多只能上传 1 张图片。'
                    })
            ...
            handleSuccess3 (response, file, fileList) {
                if (response.code === 500) {
                    ...
                    this.$Message.success('上传成功!')
                    // this.corpInfo2.logo_picture_list = this.$refs.upload3.fileList
                    this.corpInfo.logo_picture_list.push(file)
                    this.$nextTick(()=>{
            ...
            handleRemove3 (file) {
                this.corpInfo.logo_picture_list.splice(0, 1)
