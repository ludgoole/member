				...
				<Row :gutter="20">
					...
					<i-col span="24">
			          ...
			          欢迎使用代理管理，本应用将帮助您创建企业机关或者乡村帐号，并建立代理关系。
							...
							<div class="top-bar">
								<Cascader :render-format="format" v-model="locationIdArr" :data="locationList" :load-data="loadPositionDatas"
									@on-change="searchOrganization2" change-on-select style="width: 200px;">
                                </Cascader>
								<Input icon="ios-search" @on-change="searchOrganization" v-model="search"
									   :placeholder="'请输入'+type+'名称'" style="width: 200px;margin-left: 10px;" />
								...
								<Dropdown trigger="click" style="margin-left: 10px;" v-if="key !== '2' && type === '机关'">
									...
									<Dropdown-menu slot="list">
										<Dropdown-item><Button type="text" class="drop-button" @click="govRegister">创建机关帐号</Button></Dropdown-item>
										<Dropdown-item><Button type="text" class="drop-button" @click="villRegister">创建乡村帐号</Button></Dropdown-item>
									</Dropdown-menu>
								</Dropdown>
								<Dropdown trigger="click" style="margin-left: 10px;">
									...
									<Dropdown-menu slot="list">
										<Dropdown-item><Button type="text" class="drop-button" @click="download">下载代理协议模板</Button></Dropdown-item>
									</Dropdown-menu>
								</Dropdown>
							</div>
							<Row>
										...
										<TabPane label="已代理" name="0">
											...
											<Row style="margin: 10px; height: 250px;">
												...
												<Col v-if="proxyList.length !== 0" span="6" v-for="item in proxyList" :key="item.id" class="box">
													...
													<p class="ell" :title="item.name"><span>&nbsp;&nbsp;{{item.name}}</span></p>
													<p><span>&nbsp;&nbsp;登录名：{{item.account}}</span></p>
													<p><span>&nbsp;&nbsp;注册时间：{{item.createTime}}</span></p>
													<span>&nbsp;&nbsp;
										...
										<TabPane label="未被代理" name="1">
											...
											<Row style="margin: 10px; height: 250px;">
												...
												<Col v-if="notProxyList.length !== 0" span="6" v-for="item in notProxyList" :key="item.id" class="box">
													...
													<p class="ell" :title="item.orgName"><span>&nbsp;&nbsp;{{item.orgName}}</span></p>
													<p><span>&nbsp;&nbsp;登录名：{{item.loginAccount}}</span></p>
													<p><span>&nbsp;&nbsp;注册时间：{{item.createTime}}</span></p>
													<span>&nbsp;&nbsp;
		...
		created () {
		    ...
		    * */
			let params = {
				...
				approve_status: 1,
				loginAccount: JSON.parse(sessionStorage.getItem("user")).loginAccount
			}
			this.query(params)
		},
		methods: {
