                                ...
                                <Form ref="govInfo" :model="govInfo" label-position="right" :label-width="280" :rules="ruleInline">
                                    <div style="width: 1000px;" v-if="step1">
                                        <Form-item prop="gov_name" class="input" label="机关名称：">
                                            <Input v-model="govInfo.gov_name" :maxlength="50" placeholder="请输入内容" />
                                        </Form-item>
                                ...
                                <Form ref="govInfo5" :model="govInfo5" label-position="right" :label-width="280" :rules="ruleInline3">
                                    <div style="width: 1000px;" v-if="step1">
                                        <Form-item prop="qualification_certificate_picture_list" class="qualification_certificate" label="上传社会信用代码证：">
                                            <!-- <Upload ref="upload5" :show-upload-list="false"
                                                    name="upfile"
                                                    :format="['jpg', 'png']"
                                                    :max-size="2048"
                                            ...
                                            <div class="upload-add-div photo-selector" @click="handlePhotoSelectorModal('qualificationPhoto')">
                                                <Icon type="plus-circled" color="#51ad79" :size="32"></Icon>
                                                <p class="mb5">从文件管理导入</p>
                                                <p class="t-grey">支持jpg/png格式</p>
                                                <p class="t-grey">不超过2M</p>
                                            </div>
                                            <div class="new-upload-list" v-for="(item,index) in govInfo5.qualification_certificate_picture_list" :key="index">
                                                <template v-if="item.status === 'finished'">
                                                    <img style="width:140px; height: 140px;" :src="'http://'+item.response.data.picName">
                                                    <div class="demo-upload-list-cover">
                                                        <Icon type="ios-trash-outline" size="30" style="margin-top: 55px;" @click.native="handleRemove5(item)"></Icon>
                                                    </div>
                                                </template>
                                                <template v-else>
                                                    <Progress v-if="item.showProgress" :percent="item.percentage" hide-info></Progress>
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            // this.getAlbum('qualificationPhoto', this.l3)
            /*
            * TODO 表单的具体验证
            * TODO 请求后台接口获取下拉框的数据
            * */
        ...
        methods:{
            ...
            handleGetPhotoResult3 (result) {
                // 用于限制最大可上传照片的张数
                if (this.govInfo5.qualification_certificate_picture_list.length + result.length > 1) {
                    this.$Notice.warning({
                        title: '最多只能上传 1 张图片。'
                    })
                } else {
                    result.forEach(element => {
                        this.govInfo5.qualification_certificate_picture_list.push({
                            response: {
                                data: {
                                    picName: element
                                }
            ...
            handleBeforeUpload5 () {
                const check = this.$refs.upload5.fileList.length < 1 &&
                this.govInfo5.qualification_certificate_picture_list.length + this.$refs.upload5.fileList.length < 1
                if (!check) {
                    this.$Notice.warning({
                        title: '最多只能上传 1 张图片。'
                    })
            ...
            handleSuccess5(response, file, fileList) {
                if (response.code === 500){
                    ...
                    this.$Message.success('上传成功!')
                    this.govInfo5.qualification_certificate_picture_list.push(file)
            ...
            handleRemove5 (file) {
                const fileList = this.govInfo5.qualification_certificate_picture_list
                this.govInfo5.qualification_certificate_picture_list.splice(fileList.indexOf(file), 1)
                const fileList2 = this.$refs.upload5.fileList
                this.$refs.upload5.fileList.splice(fileList2.indexOf(file), 1)
            }, */
            onSelectPoint () {
