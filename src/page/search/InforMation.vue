            ...
            <Col span="18">
              <ul class="new-last" v-if="data.list!=''">
                  <li v-for="item in data.list">
                       <Row>
                          <Col span="18">
                              <router-link :to="{ path: '/InforMation/findInforMationDetail', query: { 'id': item.informationDetailId }}"><p class="h4 ell mb20">{{ item.title }}</p></router-link>
  ...
  filters: {
    ...
    fetchData: function() {
      ...
      if (
        ...
        this.url =
          "/member/inforMation/findInforMation/" +
          this.currentPage +
          "?pageSize=" +
          this.pageSize;
      } else {
        this.url =
          "/member/inforMation/findInforMationTitle/" +
          this.currentPage +
          "?title=" +
          this.$route.query.title +
          "&pageSize=" +
