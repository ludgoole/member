<template>
    <div>
        <Breadcrumb separator=">" class="mt20 mb10">
            ...
            <Breadcrumb-item href="/pro/inforMationList?flag=1">政策</Breadcrumb-item>
            ...
            <Col span="18">
              <ul class="new-last" v-if="data.list!=''">
                  <li v-for="item in data.list">
                       <Row>
                          <Col span="18">
                              <router-link :to="{ path: '/InforMation/policyDetail', query: { 'id': item.informationDetailId }}"><p class="h4 ell mb20">{{ item.title }}</p></router-link>
  ...
  methods: {
    /*fetchData: function () {
            ...
            if(this.$route.query.title==''||this.$route.query.title == undefined){
                this.url=this.$url.api + '/policy/findPolicy/' + this.currentPage +"&pageSize=" +this.pageSize
            }else{
                this.url=this.$url.api + '/policy/findPolicyTitle/' + this.currentPage + "?title=" + this.$route.query.title +"&pageSize=" +this.pageSize
            }
            axios.get(this.url)
                .then(response => {
