<template>
  <Form :label-width="150" label-position="left" class="mt20 mb20">
    ...
    <Card v-for="(item, index) in listData" :key="index" class="mb10">
      <Row>
        <Col span="22">
          <div class="b">关注的资讯类型（{{item.follow.length}}）</div>
          <Row class="mt5">
            <Col span="4" v-for="(node, nindex) in item.follow" :key="nindex"><span class="item t-orange">{{node.name}}</span></Col>
          </Row>
          <div class="mt10 mb5 b">资讯关键词（{{item.releva[0].length + item.releva[1].length + item.releva[2].length}}）</div>
          <div v-for="(node, nIndex) in item.releva" :key="nIndex">
            <template v-if="nIndex === 0"><releva-item title="物种" :span="3" :data="node"></releva-item></template>
            <template v-if="nIndex === 1"><releva-item title="产品" :span="3" :data="node"></releva-item></template>
            <template v-if="nIndex === 2"><releva-item title="服务" :span="3" :data="node"></releva-item></template>
          </div>
    ...
    :mask-closable="false">
      ...
      <template v-if="relevaShow || followShow">
        <div slot="footer" class="tc">
          <Button type="ghost" @click="handlePrev" v-if="active > 0">上一步</Button>
          <Button type="primary" @click="handleNext">下一步</Button>
        </div>
      </template>

      <div slot="footer" class="tc" v-if="active === 2">
        <Button type="ghost" @click="handlePrev">上一步</Button>
