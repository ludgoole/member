        ...
        <Row :gutter="32">
          ...
          <Col span="12">
            <Form-item prop="isShowWebsiteName" class="input" label="网站名称是否显示">
              <Select v-model="websiteInfo.isShowWebsiteName" style="width:60px;">
...
export default {
  ...
  created () {
    /* 1企业 2专家 3个人 4乡村 5机关 */
    let userType = 3
    // 根据sessionStorage中的userType给初始化网站设置时的userType赋值
    if (this.loginUser.userType === 4) {
      userType = 2
    } else if (this.loginUser.userType === 0) {
      userType = 3
    } else if (this.loginUser.userType === 1) {
      userType = 1
    } else if (this.loginUser.userType === 3) {
      userType = 5
    } else if (this.loginUser.userType === 5) {
      userType = 4
    }
