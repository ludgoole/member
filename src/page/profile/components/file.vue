<template>
    <div>
        ...
        <template v-if="fileShow">
            ...
            <Button type="ghost" @click="createdFileFolder" v-if="isshowEdit">新建文件夹</Button>
            <!-- 循环生成文档文件夹 -->
            <Row :gutter="16" class="album mt20">
                <i-col span="6" v-for="(item,index) in fileFolderList" :key="index">
        ...
        <template v-else>
            ...
            <Row :gutter="16" class="album mt20">
                ...
                <i-col span="6" v-for="(item,index) in fileList" :key="index">
                    <figure class="wrap mb10 border drop-out pointer" style="border: none;">
                        <Dropdown trigger="click" placement="right-start" class="file-drop drop-in">
                            <Icon type="arrow-down-b" style="padding:2px 5px;"></Icon>
        ...
        <!-- 上传文档 -->
        <Modal v-model="uploadFileShow" title="上传文档" width="600px" :mask-closable="false">
        ...
        <!-- 创建文件夹 -->
        <Modal v-model="createFileFolderShow" title="创建文件夹" width="600" :mask-closable="false">
        ...
        <!-- 编辑文件夹 -->
        <Modal v-model="editFileFolderShow" title="编辑文件夹" width="600" :mask-closable="false">
        ...
        <!-- 编辑文档 -->
        <Modal v-model="editFileShow" title="编辑文档" width="600" :mask-closable="false">
        ...
        methods: {
            ...
            uploadFileOk () {
                ...
                api.post('/member/media/saveMediaLibraryDetail', data).then(response => {
                    console.log('res', response)
                    if (response.code === 200) {
                        _that.uploadFileSuccessShow = true
                        _that.fileUploadInfo.toWhere = ''
                        _that.fileUploadInfo.file_list = []
                        _that.$refs.upload.fileList = []
                        _that.uploadFileShow = false
                    }
                })
            },
            uploadFileOkCancel () {
            ...
            handleFormatError (file) {
                this.$Notice.warning({
                    title: '文件格式有误',
                    desc: '文件 ' + file.name + ' 格式不正确，请选择doc,docx,xls,xlsx,rmvb,ppt,pptx,txt,pdf格式。',
                    duration: 6
                })
            },
            createFileFolderOk () {
            ...
            editFileOk () {
                ...
                api.post('/member/media/updateMediaLibraryDetail', {
                    ...
                    console.log('res', response)
                    if (response.data === 1) {
                        _that.editFileShow = false
                        _that.fileList[_that.index].title = name
                        _that.editFileInfo.file_name = ''
                    }
                })
            },
            editFileCancel () {
