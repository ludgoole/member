                    ...
                    <Row :gutter="16">
                        ...
                        <Col span="8">
                            <Form-item prop="classification" label="服务分类">
                                <Input v-model="item.classification"></Input>
                            </Form-item>
                        </Col>
                        <Col span="8">
                            <Form-item prop="creatTime" label="创建时间">
                                <DatePicker v-model="item.creatTime" type="date" @on-change="change"  :options="options3" placement="bottom-end"></DatePicker>
                                <!-- <Input v-model="item.creatTime"></Input>    -->
                            </Form-item>
                        </Col>
                        <Col span="24">
        ...
        methods: {
            getData(val){
                ...
                for(var i = 0 ;i < data.length ; i++){
                    var index = `index${i}`
                    this.$refs[index][0].handleGive(data[i].pictureList)
                }
            },
            change(e){
                this.data[this.index].creatTime = e
            },
             //表单验证
            handleSubmit () { 
                this.submit = true               
            ...
            handleGetServiceResult(classify, result) {
                if (result.length) {
                    ...
                    this.data[this.index].serviceName = arr.join(' ')
                } else {
