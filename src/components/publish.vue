			...
			<!-- <Tabs @on-click="showMydynamic">
				<Tab-pane :label="item.columnName" v-for="(item,index) in options.columnSetting" v-show="indexpane <= 2"> -->
					<Tabs type="card"  @on-click="changeLabel">
						...
						<Button type="ghost" @click="handleTabsAdd" size="small" slot="extra" :disabled="tabs===10" class="tabadd">+
                          </Button>
						<Button type="ghost" @click="handleTabsDel" size="small" slot="extra" class="tabadd"  v-show="isHide" >-
						</Button>
					</Tabs>
				<!-- </Tab-pane>
			</Tabs> -->
...
<script>
	...
	export default {
		...
		created() {
			...
			// this.mydynamic.author = JSON.parse(sessionStorage.getItem("user")).displayName
			// 取地址
		...
		methods: {
            ...
            handleGetBookPage(data){
                ...
                let fileData = data
                axios.post('http://192.168.7.42:8085/imgbase64/upload',
                    {
                        fileName: fileName,
                        fileData: fileData
                    })
			...
			handleGetBase64 (data) {
				 data.getCropData(d =>{
                  ...
                  /*   this.$api.post('http://192.168.7.42:8085/imgbase64/upload', {fileName:fileName,fileData:fileData}).then(response => {
			...
			showMydynamic(index = 0) {
				...
				let columnName=this.options.columnSetting[index].name
                this.infoType =[]
                this.mydynamic.type = ''
				if('政策' === columnName){
					this.showPol()
            ...
            handleTabsDel () {
				this.$Modal.confirm({
					...
					content: '确定删除该标签？',
					onOk: () => {
						let columnName = this.options.columnSetting[this.indexpane].name
						this.allSmallPaneData.forEach(element => {
							if (element.typeName === columnName) {
