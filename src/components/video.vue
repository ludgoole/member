        		...
        		<Row>
        			<Col span="10"  class="up-video1" v-for="(item,index) in videoList" :key="index">
        				 <video :src="item.url" width="300" height="112%" controls="controls"/>
                           <Button type="ghost" shape="circle" icon="close-round" @click.native="handleRemove(item)" class="demo-upload-list-cover"></Button>
  ...
  methods: {
    ...
    handleSuccess(response, file, fileList) {
      if (response.code === 500) {
        ...
        this.videoList.push({url:'http:'+response.data.picName,describe:'',type:0})
        this.saveDescribe()
