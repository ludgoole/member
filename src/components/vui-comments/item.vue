...
<div>
  <ul class="vui-comment-list">
    <li class="vui-comment-item" v-for="(item, index) in data.list">
      ...
      <div>
        <Button type="text" size="small" @click="handleLike(item)"><Icon :size="14" type="thumbsup"></Icon> {{item.thumb_up_num}}</Button>
        ...
        <Button type="text" size="small" v-if="account != item.user_account" @click="handleReplyBtn(item, index)"><Icon :size="15" type="reply"></Icon> <span v-if="!item.replyBoxShow">回复</span><span v-if="item.replyBoxShow">收起回复</span> </Button>
        <Poptip
          v-else
          transfer
          confirm
          ...
          <Button type="text" size="small"><Icon :size="15" type="trash-b"></Icon> 删除</Button>
        </Poptip>
      </div>
